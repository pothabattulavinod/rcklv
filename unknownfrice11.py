...
# Filter RCs with "Unknown" status
data = [entry for entry in previous_data if entry.get("transaction_status") == "Unknown"]
total_rcs = len(data)

if not data:
    print("✅ No RCs with 'Unknown' status found. Nothing to check.")
    exit(0)

print(f"🔍 Rechecking {total_rcs} RCs marked as 'Unknown'...\n")
...
# ✅ Only update entries that were "Unknown"
updated_data = []
updated_map = {entry["CARDNO"]: entry for entry in transaction_data}

for entry in previous_data:
    rcno = entry.get("CARDNO")
    if rcno in updated_map:  # Replace only updated ones
        updated_data.append(updated_map[rcno])
    else:
        updated_data.append(entry)  # Keep the rest unchanged

# Save the updated JSON
with open(output_file, "w", encoding="utf-8") as f:
    json.dump(updated_data, f, indent=4, ensure_ascii=False)

print(f"\n✅ Recheck complete. Updated results saved in '{output_file}'. Only 'Unknown' entries were modified.")
